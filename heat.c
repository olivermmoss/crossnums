...
bool visited[solutions][N][N]; //debugging only
...
int main(){
            ...
            if(myPtr!=NULL && strcmp(myPtr, "v")==0){
                while(myPtr != NULL) {
                    ...
                    if (*endptr != '\0') {
						...
						// visited[curTable][i][j] = true;
						// visited[curTable][j][i] = true;

						if(j == (i+1)%N) assert(rotstorage[curTable][i][j]);
    ...
    fclose(fptr);
		...
		for(int a = 0; a < solutions; a++) {
			...
			//checks post-bankers graph as expected
			printf("GRAPH %d: \n", a);
